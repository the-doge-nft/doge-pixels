...
const CircleLineProgress = ({items, activeIndex}: Stepper) => {
    ...
    return <Box position={"relative"} mb={5} mx={5}>
        <Flex w={"100%"} alignItems={"center"} justifyContent={"space-between"}>
            {items.map((item, index, arr) => <ProgressCircle
                key={item.key}
                title={item.title}
...
const ProgressCircle = ({title, isActive}: {title: string, isActive: boolean}) => {
    return <Flex direction={"column"} alignItems={"center"} position={"relative"} zIndex={2}>
