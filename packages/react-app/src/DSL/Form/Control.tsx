...
const Control = ({name, children, label, horizontal = false, ...rest}: ControlProps) => {
    ...
    return <FormControl id={name} w={"100%"} isInvalid={meta.error && meta.touched} {...rest}>
        <Grid templateColumns={horizontal ? "1fr 1fr" : undefined} w={"100%"}>
            <GridItem>
                {label && <FormLabel htmlFor={name} mb={1}>
                    <Typography variant={TVariant.Detail14} color={"gray.600"}>{label}</Typography>
                </FormLabel>}
            </GridItem>
            <GridItem>
                {children}
