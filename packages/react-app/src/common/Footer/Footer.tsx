...
const FooterItem: React.FC<{title: string, items: {title: string, link: string}[]}> = ({title, items}) => {
  ...
  return <Box>
    <Typography block mb={3} opacity={0.5} color={lightOrDarkMode(colorMode, "black", "white")} fontWeight={"bold"} variant={TVariant.ComicSans14}>
      {title}
    </Typography>
    <Grid templateRows={"1fr 1fr 1fr"}>
      {items.map(item => <Link opacity={0.5} size="sm" target={"_blank"} variant={Type.ComicSans} href={item.link}>{item.title}</Link>)}
    </Grid>
  </Box>
}
