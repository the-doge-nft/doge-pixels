...
const CowSwap: React.FC<{ store: MintPixelsDialogStore }> = observer(({store}) => {
    ...
    return (
        <Box>
            ...
            {store.cowSwapOrderID && <Flex justifyContent="center" my={10}>
              <Link href={`${CowStore.BASE_EXPLORER_URL}/${store.cowSwapOrderID}`} isExternal>
                View Order Status</Link>
            </Flex>}

            {store.cowSwapOrderID && <Flex justifyContent={"center"}>
            ...
            {isOpen && store.recentQuote && <Box mt={2}>
                {store.srcCurrency !== "DOG" && <>
                  ...
                  <Flex alignItems={"center"} justifyContent={"flex-end"} mt={2}>
                    ...
                    <Typography color={colorMode === "light" ? "yellow.800" : "purple.50"} variant={TVariant.ComicSans14}>Powered by CoW Protocol</Typography>
                  </Flex>
                </>}
            </Box>}
        </Box>
