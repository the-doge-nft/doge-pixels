...
export const MintPixelsInput: React.FC<{ store: MintPixelsDialogStore }> = ({ store }) => {
  ...
  const PixelInput = useCallback(() => {
        ...
        renderLeftOfValidation={() => {
          return (
            <Box>
              ...
              {store.recentQuote && !store.isLoading && (
                <Box mt={4} display={"flex"} flexDirection={"column"}>
                  <Typography variant={TVariant.ComicSans14}>
                    {store.recentQuote.computedPixelCount} Pixel{store.pixelCount === 1 ? "" : "s"} ={" "}
    ...
    showLabel,
    store,
    store.recentQuote?.maxPixelAmount,
    store.srcCurrency,
    store.srcCurrencySelectItems,
    store.isLoading,
    store.recentQuote,
  ]);
  return <PixelInput />;
