...
const runTests = (withUpgrade) => {
  ...
  describe(suiteName, function () {
    ...
    let FEES_ACCOUNT_DEV, FEES_ACCOUNT_PLEASR;
    ...
    function getShardIndex(indexWithOffset) {
      ...
      const shard =
        1 + Math.floor((indexWithOffset - INDEX_OFFSET) / SHARD_SIZE);
      return shard;
    }
    ...
    before("Should deploy contract", async function () {
      ...
      await DOG20.__DOG20_init(
        signers.map((item) => item.address),
        DOG_TO_PIXEL_SATOSHIS.mul(MOCK_SUPPLY)
      );
      ...
      await Promise.all([PX.setDOG_TO_PIXEL_SATOSHIS(DOG_TO_PIXEL_SATOSHIS)]);
    ...
    async function mintPupperWithValidation(
      signer,
      mintQty = 1,
      _shouldRevertWithMessage = undefined
    ) {
      ...
      await DOG20.connect(signer).approve(
        PX.address,
        DOG_TO_PIXEL_SATOSHIS.mul(mintQty)
      );
      let tx;
      const foo = () => {
      ...
      if (_shouldRevertWithMessage) {
        tx = await expect(foo()).to.be.revertedWith(_shouldRevertWithMessage);
        return;
      } else {
        tx = await foo();
      }
      expect(await DOG20.balanceOf(PX.address)).to.equal(
        pxDog20BalanceBefore.add(DOG_TO_PIXEL_SATOSHIS.mul(mintQty))
      );
      expect(await DOG20.balanceOf(signer.address)).to.equal(
        addrDog20BalanceBefore.sub(DOG_TO_PIXEL_SATOSHIS.mul(mintQty))
      );
      expect(await PX.balanceOf(signer.address)).to.equal(
        addrPXBalanceBefore.add(mintQty)
      );
      expect(await PX.puppersRemaining()).to.equal(
        supplyPXBalanceBefore.sub(mintQty)
      );
    ...
    async function burnPupperWithValidation(
      signer,
      pupper,
      _shouldRevertWithMessage = undefined
    ) {
      ...
      const addrDog20BalanceBefore = await DOG20.balanceOf(signer.address);
      const pxDog20BalanceBefore = await DOG20.balanceOf(PX.address);
      const feesDevDog20BalanceBefore = await DOG20.balanceOf(
        FEES_ACCOUNT_DEV.address
      );
      const feesPleasrDog20BalanceBefore = await DOG20.balanceOf(
        FEES_ACCOUNT_PLEASR.address
      );
      const addrPXBalanceBefore = await PX.balanceOf(signer.address);
      const supplyPXBalanceBefore = await PX.puppersRemaining();
      ...
      if (_shouldRevertWithMessage) {
        tx = await expect(
          PX.connect(signer).burnPuppers([pupper])
        ).to.be.revertedWith(_shouldRevertWithMessage);
        return;
      } else {
      ...
      expect(await DOG20.balanceOf(PX.address)).to.equal(
        pxDog20BalanceBefore.sub(DOG_TO_PIXEL_SATOSHIS)
      );
      expect(await DOG20.balanceOf(signer.address)).to.equal(
        addrDog20BalanceBefore.add(DOG_TO_PIXEL_SATOSHIS.mul(99).div(100))
      );
      expect(await DOG20.balanceOf(FEES_ACCOUNT_DEV.address)).to.equal(
        feesDevDog20BalanceBefore.add(
          DOG_TO_PIXEL_SATOSHIS.mul(BURN_FEES_PERCENT_DEV).div(100).div(100)
        )
      );
      expect(await DOG20.balanceOf(FEES_ACCOUNT_PLEASR.address)).to.equal(
        feesPleasrDog20BalanceBefore.add(
          DOG_TO_PIXEL_SATOSHIS.mul(BURN_FEES_PERCENT_PLEASR).div(100).div(100)
        )
      );

      expect(await PX.balanceOf(signer.address)).to.equal(
        addrPXBalanceBefore.sub(1)
      );
      expect(await PX.puppersRemaining()).to.equal(
        supplyPXBalanceBefore.add(1)
      );
      //
      // let receipt = await tx.wait();
      // for (let i = 0; i < receipt.events.length; ++i) {
    ...
    context("supply", function () {
      ...
      it("pupper can be transferred from token owner to another address", async function () {
        ...
        await PX.connect(addr1).transferFrom(
          addr1.address,
          addr2.address,
          tokenId
        );
        ...
        await PX.connect(addr2).transferFrom(
          addr2.address,
          addr1.address,
          tokenId
        );
      ...
      it("try to mint with sender having no $DOG balance", async function () {
        ...
        await mintPupperWithValidation(
          signer4,
          1,
          ERROR_D20_TX_EXCEEDS_BALANCE
        );
      });
      it("burnPuppers empty array throws", async function () {
        await expect(PX.connect(addr1).burnPuppers([])).to.be.revertedWith(
          "Empty puppers"
        );
      });
      it("burnPuppers burns puppies", async function () {
        ...
        expect(
          (await PX.puppersRemaining()).toNumber()
      ...
      it("burn all supply", async function () {
        ...
        for (let i = 0; i < arr.length; ++i) {
          ...
          await burnPupperWithValidation(
            signer,
            tokenId,
      ...
      it("mint/burn cycle", async function () {
        ...
        for (let i = 0; i < cyclesQty; ++i) {
          ...
          if (Math.floor(Math.random() * 100000) % divider) {
            if (remaining === 0) {
              await mintPupperWithValidation(
                randFromArray(signers.slice(0, 2)),
                ERROR_NO_PX_REMAINING
              );
            }
          } else {
